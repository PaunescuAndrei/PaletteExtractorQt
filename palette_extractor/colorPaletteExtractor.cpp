...
void reduceColor_kmeans(const cv::Mat& src, cv::Mat& dst, int K_clusters)
{
    ...
    cv::Mat labels;
    cv::Mat centers;
    cv::TermCriteria criteria = cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 20, 1.0);
    cv::kmeans(kmeanPoints, K_clusters, labels, criteria, 3, cv::KmeansFlags::KMEANS_PP_CENTERS, centers);
